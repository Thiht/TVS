...
def generate_url(name, season_number, episode_number):
    """
...
if args.search:
        ...
        for season in list_episodes["seasons"]:
            ...
            for episode in list_episodes["seasons"][season]:
                ...
                if args.generate_url:
                    url = generate_url(list_episodes["name"], season, episode)
                    print("url: " + url)
    except ValueError as e:
        print(e)
    ...
    try:
        ...
        if "number" in next_episode:
            ...
            if args.generate_url:
                url = generate_url(next_episode["name"], next_episode["season"], next_episode["number"])
                print("url: " + url)
        else:
    ...
    for name, data in list(check.items()):
        ...
        if args.generate_url:
            url = generate_url(name, data["season"], data["number"])
            print("url: " + url)
